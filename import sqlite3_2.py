...
maxPLM = cur.fetchall()
maxx=maxPLM[0][0]
print("Maximum Payload is =")
print(maxx)
cur.execute('''SELECT DISTINCT Booster_Version from SPACEXTABLE WHERE PAYLOAD_MASS__KG_ = ?''',(maxx,))
SpeBosstMx = cur.fetchall()
print(SpeBosstMx)

cur.execute('''SELECT Landing_Outcome, Booster_Version, Launch_Site
FROM SPACEXTBL
WHERE SPACEXTBL.Landing_Outcome LIKE 'Failure (ground pad)'
AND SPACEXTBL.Date BETWEEN '2015-01-01' AND '2015-12-31';''')
cur.fetchall()

cur.execute('''SELECT Landing_Outcome, Booster_Version, Launch_Site
FROM SPACEXTBL
WHERE SPACEXTBL.Landing_Outcome LIKE 'Failure (drone ship)'
AND SPACEXTBL.Date BETWEEN '2015-01-01' AND '2015-12-31';''')
cur.fetchall()

cur.execute('''SELECT Landing_Outcome, COUNT(Landing_Outcome)
FROM SPACEXTBL
WHERE SPACEXTBL.Date BETWEEN '2010-06-04' AND '2017-03-20' GROUP BY Landing_Outcome ORDER BY COUNT(Landing_Outcome) DESC;''')
cur.fetchall()
